...
using tflite_exploder::read_entire_file;
using tflite_exploder::write_entire_file;
...
int main(int argc, char **argv) {
    ...
    for (size_t subgraph_index = 0; subgraph_index < subgraphs->size(); subgraph_index++) {
        ...
        for (const tflite::Operator *op : *subgraph->operators()) {
            ...
            auto new_subgraph = std::make_unique<tflite::SubGraphT>();
            ...
            // signature_defs is optional -- not sure if we need it for our purposes.
            ...
            outpath << output_dir << "/";
            if (subgraphs->size() > 1) {
                // If multiple subgraphs, prepend with subgraph index
                outpath << std::setfill('0') << std::setw(3) << subgraph_index << "_";
            }
            outpath << std::setfill('0') << std::setw(3) << op_index << "." << op_name << ".tflite";
